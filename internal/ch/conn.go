...
func ConnectLoop(ctx context.Context, dsn string, logger *slog.Logger) (conn driver.Conn, closeFunc func() error, err error) {
	...
	timeoutExceeded := time.After(defaultTimeout)

	for {
		select {
		case <-timeoutExceeded:
			return nil, nil, fmt.Errorf("clickhouse: db connection failed after %s timeout", defaultTimeout)
		case <-ticker.C:
			if err = db.Ping(ctx); err == nil {
				return db, db.Close, nil
			}
