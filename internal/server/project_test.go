...
func TestServer_HandleProjectDetails(t *testing.T) {
	...
	t.Run("Go Zap: Project issue naming and output in project details without error", func(t *testing.T) {
		...
		assert.JSONEq(t, `{"id":"0018ce1ba9d34f688814c938b74d8e14"}`, w.Body.String())
		...
		var (
			expectedType = "My error message at 2025-10-09T23:59:42+03:00"
			expectedMsg  = "(No error message)"
		)

		assert.Equal(t, http.StatusOK, w.Code)
		assert.Equal(t, expectedType, issueType)
		assert.Equal(t, expectedMsg, issueMsg)
